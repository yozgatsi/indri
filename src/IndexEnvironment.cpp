...
/*
  std::cout << "indri::api::IndexEnvironment::_createAnnotators fileName = " << fileName << ", fileClass = " << fileClass << std::endl;
*/
  ...
  if (fileName.empty()) {
    ...
    // case: addFile
    relativePath = indri::file::Path::relative( _documentRoot, fileName );
  }

/*
  std::cout << "indri::api::IndexEnvironment::_createAnnotators relativePath = " << relativePath << std::endl;
*/
...
/*
  std::cout << "indri::api::IndexEnvironment::_createAnnotators _offsetAnnotationsRoot = " << _offsetAnnotationsRoot << std::endl;
*/
...
/*
  std::cout << "indri::api::IndexEnvironment::_createAnnotators _offsetMetadataRoot = " << _offsetMetadataRoot << std::endl;
*/
  ...
  if( !parser || !iterator ) {
    ...
    try {
      ...
      while( (document = iterator->nextDocument()) ) {
        ...
        /*
         * can't know for sure that we have the docno element until after the annotators have been applied
         * check if this document is in the index already, duplicates must not be indexed
         */
...
/*
          std::cout << "indri::api::IndexEnvironment::addFile metadata[";
          std::cout << (const char *)parsed->metadata[i].key << "] = ";
          std::cout << (const char *)parsed->metadata[i].value;
          std::cout << std::endl;
*/
        ...
        if(_blackedDocs.find(docIDStr)!=_blackedDocs.end())
            continue;
/*
        std::cout << "indri::api::IndexEnvironment::addFile docIDStr = " << docIDStr << std::endl;
*/
        ...
        if (ids.size() == 0)  {
          ...
          // duplicete, don't index, return 0 as doc id
          //documentID = -1; addFile return is void
          std::cout << "indri::api::IndexEnvironment::addFile skip indexing, document is a duplicate."<< std::endl;
        }
...
lemur::api::DOCID_T indri::api::IndexEnvironment::addString( const std::string& documentString, const std::string& fileClass, const std::vector<indri::parse::MetadataPair>& metadata ) {
  ...
  indri::parse::Conflater* conflater = 0;
  std::string nothing = "";
  lemur::api::DOCID_T documentID = 0; // invalid

  std::vector<indri::parse::Transformation*> annotators = _createAnnotators( nothing, fileClass, &conflater );
  ...
  for( size_t i=0; i<parsed->metadata.size(); i++ ) {
/*
    std::cout << "indri::api::IndexEnvironment::addString metadata[";
    std::cout << (const char *)parsed->metadata[i].key << "] = ";
    std::cout << (const char *)parsed->metadata[i].value;
    std::cout << std::endl;
*/
...
/*
  std::cout << "indri::api::IndexEnvironment::addString docIDStr = " << docIDStr << std::endl;
*/
  // look up the doc id
  std::vector<lemur::api::DOCID_T> ids = _repository.collection()->retrieveIDByMetadatum("docno", docIDStr);
  // if not found, add the document
  if (ids.size() == 0)  {
    documentID =_repository.addDocument( parsed );
    _documentsIndexed++;
  } else {
    // duplicete, don't index, return 0 as doc id
    documentID = -1;
    std::cout << "indri::api::IndexEnvironment::addString skip indexing, document is a duplicate."<< std::endl;
  }
...
bool indri::api::IndexEnvironment::documentExists( lemur::api::DOCID_T documentID ) {
  return _repository.documentExists( documentID );
}
